...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        #當使用者輸入的文字為faq中有的項目(key)時，將結果(value)定義成bot_msg
        if user_msg in faq:
            bot_msg = faq[user_msg]
        elif user_msg.lower() in ["menu","選單","首頁"]:
            bot_msg = menu
        elif user_msg in table:
            buy = table[user_msg]["buy"]
            sell = table[user_msg]["sell"]
            bot_msg = TextMessage(text=f"{user_msg}買價：{buy}\n賣價：{sell}\n來源：台灣銀行")
        else:
